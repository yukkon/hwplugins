...
const onclick = async (e) => {
  ...
  const x = loader.cachedImages.get(
    `dialog_season_adventure_tiles.rsx/bg_season_1400_box`
  );
  ctx.drawImage(x.image, x.x, x.y, x.width, x.height, 0, 0, x.width, x.height);
  let src = g.toDataURL();
  // #endregion

  container.innerHTML = `<div><input type="checkbox" id="showProcessed" /><label for="showProcessed">Показывать пройденные</label></div><div style='background-image: url(${src});background-size:100% 100%; overflow:hidden;'><canvas class='dragme' id='hexagonCanvas' style='border: 1px solid black; position: relative;'></canvas></div>`;
  ...
  const processed = await Send(
    `{"calls":[{"name":"seasonAdventure_getMapState","args":{"seasonAdventureId": ${id}},"ident":"body"}]}`
  )
    .then((r) => (!r.error ? r.results[0].result.response.levels : []))
    .then((arr) =>
      arr
        .filter((l) => l.steps.length > 0)
        .reduce((acc, v) => {
          acc[v.id] = v.steps[0].isProcessed;
          return acc;
        }, {})
    );
...
class A {
  ...
  drawBoard(size = 40) {
    ...
    this.canvas.width = this.ctx.width =
      4 * this.hexRect.w + (this.offset.maxx - this.offset.minx);
    this.canvas.height = this.ctx.height =
      2 * this.hexRect.h + (this.offset.maxy - this.offset.miny);
    ...
    if (el?.reward?.length > 0) {
      ...
      el.reward.forEach(({ k: type, v }) => {
        ...
        if (!!im) {
          let rect = {
            ...
            y: y - h / (2 * c),
            w: h / c,
            h: Math.ceil(this.hexRect.w / c) + 4,
          };
          if (el.processed && this.showProcessed) {
          ...
          let b = this.loader.cachedImages.get(
            `dialog_basic.rsx/${this.loader.lib.enum.itemColor[5].frameAssetTexture}`
          );
          if (!!item?.color) {
            ...
            b = this.loader.cachedImages.get(
              `dialog_basic.rsx/${
                this.loader.lib.enum.itemColor[item.color][a]
              }`
            );
          }
          this.ctx.drawImage(
          ...
          let mm = Intl.NumberFormat("en-US", {
            notation: "compact",
          ...
          this.ctx.fillRect(
            rect.x + rect.w / 2 - wt / 2 - 4,
            y + this.hexRect.h - Math.ceil(this.hexRect.w) - 2,
            wt + 8,
            Math.ceil(this.hexRect.w)
          );
      ...
      if (!!el?.visible) {
        let im = this.loader.cachedImages.get(
          `dialog_season_adventure_tiles_new.rsx/${el?.visible}_image`
        );
        //"js/gui/dialog_season_adventure_tiles_new.rsx"
        if (im) {
          this.ctx.drawImage(
            im.image,
            im.x,
